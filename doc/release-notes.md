...
BonorumCoin version *version* is now available from:  <https://github.com/bonorumcoin-project/bonorumcoin/releases>
...
BonorumCoin is extensively tested on multiple operating systems using the Linux kernel, macOS 10.10+, and Windows 7 and later.

Microsoft ended support for Windows XP on [April 8th, 2014](https://www.microsoft.com/en-us/WindowsForBusiness/end-of-xp-support), No attempt is made to prevent installing or running the software on Windows XP, you can still do so at your own risk but be aware that there are known instabilities and issues. Please do not report issues about Windows XP to the issue tracker.

Apple released it's last Mountain Lion update August 13, 2015, and officially ended support on [December 14, 2015](http://news.fnal.gov/2015/10/mac-os-x-mountain-lion-10-8-end-of-life-december-14/). BonorumCoin software starting with v3.2.0 will no longer run on MacOS versions prior to Yosemite (10.10). Please do not report issues about MacOS versions prior to Yosemite to the issue tracker.

BonorumCoin should also work on most other Unix-like systems but is not frequently tested on them.
...
You can read more details about this extensive work in ([PR #954](https://github.com/BonorumCoin-Project/BonorumCoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/BonorumCoin-Project/BonorumCoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [BonorumCoin Wiki](https://github.com/BonorumCoin-Project/BonorumCoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/BonorumCoin-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/BonorumCoin-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/BonorumCoin-Project/BonorumCoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 BONO, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zBONO Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zBONO have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zBONO with public spends version 4 ([PR #936](https://github.com/BonorumCoin-Project/BonorumCoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zBONO), version 4 spends will also be public. A full technical writeup is available on the [BonorumCoin Wiki](https://github.com/BonorumCoin-Project/BonorumCoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/BonorumCoin-Project/BonorumCoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/BonorumCoin-Project/BonorumCoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/BonorumCoin-Project/BonorumCoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/BonorumCoin-Project/BonorumCoin/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-zbono-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - BONO balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - BONO balance held in cold staking addresses.
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~bonorumcoin) repository, we are now offering a [Snap package](https://snapcraft.io/bonorumcoin-core) as quick way to install and update a BonorumCoin wallet.
