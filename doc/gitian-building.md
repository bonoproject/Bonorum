...
*Setup instructions for a gitian build of BonorumCoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BonorumCoin
...
to the BonorumCoin GitHub release page.
...
- [Building BonorumCoin](#building-bonorumcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bonorumcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bonorumcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bonorumcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bonorumcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bonorumcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bonorumcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bonorumcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bonorumcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bonorumcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bonorumcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bonorumcoin-osx-*-build.assert
```
