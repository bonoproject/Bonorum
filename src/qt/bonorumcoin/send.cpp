...
#include "qt/bonorumcoin/send.h"
#include "qt/bonorumcoin/forms/ui_send.h"
#include "qt/bonorumcoin/addnewcontactdialog.h"
#include "qt/bonorumcoin/qtutils.h"
#include "qt/bonorumcoin/sendchangeaddressdialog.h"
#include "qt/bonorumcoin/optionbutton.h"
#include "qt/bonorumcoin/sendconfirmdialog.h"
#include "qt/bonorumcoin/myaddressrow.h"
#include "qt/bonorumcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(BonorumCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (BONO)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a BonorumCoin address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 BONO");
	...
	setCssProperty(coinIcon, "coin-icon-bono");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_BONO, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any BONO to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress bonoAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(bonoAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(bonoAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
