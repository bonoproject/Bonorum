...
#include "qt/bonorumcoin/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of only sharing a BonorumCoin address, you can create a Payment Request message which bundles up more information than is contained in just a BonorumCoin address."));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BONO");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " BONO";
        } else {
