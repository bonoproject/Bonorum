...
#include "qt/bonorumcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/bonorumcoin/lockunlock.h"
#include "qt/bonorumcoin/qtutils.h"
#include "qt/bonorumcoin/receivedialog.h"
...
TopBar::TopBar(BonorumCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopBono}, "amount-small-topbar");
    setCssProperty(ui->labelAmountBono, "amount-topbar");
    setCssProperty({ui->labelPendingBono}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("BonorumCoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // BONO Total
    CAmount bonoAvailableBalance = balance;

    // Set
    QString totalBono = GUIUtil::formatBalance(bonoAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopBono->setText(totalBono);

    // Expanded
    ui->labelAmountBono->setText(totalBono);

    ui->labelPendingBono->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureBono->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
