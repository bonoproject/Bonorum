...
#include "qt/bonorumcoin/forms/ui_addresseswidget.h"
#include "qt/bonorumcoin/addresslabelrow.h"
#include "qt/bonorumcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(BonorumCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a BonorumCoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress bonoAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(bonoAdd.Get(), label.toUtf8().constData(),
                bonoAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
