...
#include "qt/bonorumcoin/dashboardwidget.h"
#include "qt/bonorumcoin/forms/ui_dashboardwidget.h"
#include "qt/bonorumcoin/sendconfirmdialog.h"
#include "qt/bonorumcoin/txrow.h"
...
DashboardWidget::DashboardWidget(BonorumCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BONO staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareBono, "square-chart-bono");
    setCssProperty(ui->labelBono, "text-chart-bono");
    ...
    ui->labelAmountBono->setText("0 BONO");
    setCssProperty(ui->labelAmountBono, "text-stake-bono-disable");
    ...
    ui->btnHowTo->setText(tr("How to get BONO"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("BONO")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors(){
    ...
    if(isLightTheme()){
        ...
        gridLineColorX = QColor(22,11,12);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bono = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBono += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBono.append(bono);

        int max = std::max(bono, bono);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("BONO");
    set0->setColor(QColor(168,12,34));
    ...
    set0->append(chartData->valuesBono);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBono > 0) {
        setCssProperty(ui->labelAmountBono, "text-stake-bono");
    } else {
        setCssProperty(ui->labelAmountBono, "text-stake-bono-disable");
    }
    forceUpdateStyle({ui->labelAmountBono});
    ui->labelAmountBono->setText(GUIUtil::formatBalance(chartData->totalBono, nDisplayUnit));
