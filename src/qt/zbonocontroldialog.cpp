...
#include "zbonocontroldialog.h"
#include "ui_zbonocontroldialog.h"
...
std::set<std::string> ZBonoControlDialog::setSelectedMints;
std::set<CMintMeta> ZBonoControlDialog::setMints;

bool CZBonoControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBonoControlDialog::COLUMN_DENOMINATION || column == ZBonoControlDialog::COLUMN_VERSION || column == ZBonoControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBonoControlDialog::ZBonoControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBonoControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBONO Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZBono->setProperty("cssClass", "text-main-purple");
    ui->labelZBono_int->setProperty("cssClass", "text-main-purple");
...
ZBonoControlDialog::~ZBonoControlDialog()
{
    delete ui;
}

void ZBonoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBonoControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBonoControlWidgetItem* itemDenom(new CZBonoControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBonoControlWidgetItem *itemMint = new CZBonoControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zBONO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBONO seed used to mint this zBONO is not the same as currently hold in the wallet";
            else
...
void ZBonoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBonoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBono_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZBonoControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBonoControlDialog::GetSelectedMints()
{
...
void ZBonoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
