                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select BONO Denominations to Spend");
    ...
    if (ui->radioListMode->isChecked()) {
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++) {
            ...
            if (model->isLockedCoin(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt())) {
                ...
                item->setIcon(COLUMN_CHECKBOX, QIcon("://ic-check-locked"));
            }
            updateLabelLocked();
        }
        ui->treeWidget->setEnabled(true);
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::BONO;
    ...
    QString toolTip4 = tr("Can vary +/- %1 ubono per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show BonorumCoin address. In tree mode, address is not shown again for direct wallet address outputs
