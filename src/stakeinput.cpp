...
CZBonoStake::CZBonoStake(const libzerocoin::CoinSpend& spend)
{
...
int CZBonoStake::GetChecksumHeightFromMint()
{
...
int CZBonoStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZBonoStake::GetChecksum()
{
    return nChecksum;
}

// The zBONO block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZBonoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZBonoStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zBONO staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZBonoStake::GetUniqueness()
{
    //The unique identifier for a zBONO is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZBonoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZBonoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zBONO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBONOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBONO output", __func__);
    vout.emplace_back(outReward);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBONOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBONO output", __func__);
...
//!BONO Stake
bool CBonoStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBonoStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CBonoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBonoStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CBonoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CBonoStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CBonoStake::GetUniqueness()
{
    //The unique identifier for a BONO stake is the outpoint
...
CBlockIndex* CBonoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
