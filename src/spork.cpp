...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       5000),          // 5000 BONO
    ...
    MAKE_SPORK_DEF(SPORK_10_MASTERNODE_PAY_UPDATED_NODES,   0),             // ON
...
// BonorumCoin: on startup load spork values from previous session if they exist in the sporkDB
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        // BonorumCoin: add to spork database.
