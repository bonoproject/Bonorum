...
CzBONOWallet::CzBONOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbono seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBONOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBONOSeed_deprecated()) {
                    LogPrintf("%s: Updated zBONO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbono seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbono wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBONOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBONOCount(nCountLastUsed);
    else if (!walletdb.ReadZBONOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBONOWallet::Lock()
{
    seedMaster = 0;
}

void CzBONOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBONOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBONO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBONOWallet::LoadMintPoolFromDB()
{
...
void CzBONOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBONOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBONOWallet::SyncWithChain(bool fGenerateMintPool)
{
...
void CzBONOWallet::SeedToZBONO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBONOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBONOWallet::GenerateDeterministicZBONO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBONOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBONOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBONOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
