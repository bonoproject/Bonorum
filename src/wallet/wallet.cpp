...
/**
 * Fees smaller than this (in ubono) are considered zero fee (for transaction creation)
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight)
{
	...
	if (GetBoolArg("-bonostake", true)) {
		for (const COutput &out : vCoins) {
			...
			std::unique_ptr<CBonoStake> input(new CBonoStake());
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
	...
	vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BONO collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
	...
	std::string strFail = "";
	std::vector<std::pair<CScript, CAmount> > vecSend;
	vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BONO collateral
			...
			while (true) {
				...
				if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
					if (coin_type == ALL_COINS) {
						...
						strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 BONO.");
					}
					else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
						strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 BONO.");
					}
					else {
				...
				if (nChange > 0) {
					...
					// change transaction isn't always pay-to-bonorumcoin-address
					CScript scriptChange;
...
)
{
	...
	for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
		...
		//Masternode payment
		FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZBONO());
...
bool CWallet::CreateZBONOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
	if (!spend.Verify(accumulator)) {
		receipt.SetStatus(_("The transaction did not verify"), ZBONO_BAD_SERIALIZATION);
	...
	if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
		//Tried to spend an already spent zBONO
		receipt.SetStatus(_("The coin spend has been used"), ZBONO_SPENT_USED_ZBONO);
	...
	CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
	...
	receipt.SetStatus(_("Transaction Mint Started"), ZBONO_TXMINT_GENERAL);
	...
	for (auto &it : mapMintsSelected) {
		...
		if (!GenerateAccumulatorWitness(&coinWitness, mapAccumulators, pindexCheckpoint)) {
			receipt.SetStatus(_("Couldn't generate the accumulator witness"),
				ZBONO_FAILED_ACCUMULATOR_INITIALIZATION);
		...
		if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
			...
			if (!mint.GetKeyPair(key))
				return error("%s: failed to set zBONO privkey mint version=%d", __func__, nVersion);
		...
		try {
			...
			if (!CheckCoinSpend(spend, accumulator, receipt)) {
				receipt.SetStatus(_("CoinSpend: failed check"), ZBONO_SPEND_ERROR);
				return error("%s : %s", __func__, receipt.GetStatusMessage());
			}
			...
			receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZBONO_INVALID_WITNESS);
			return error("%s : %s", __func__, receipt.GetStatusMessage());
		}
	}
	...
	receipt.SetStatus(_("Spend Valid"), ZBONO_SPEND_OKAY); // Everything okay
	...
	CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
	...
	for (auto &it : mapMintsSelected) {
		...
		if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
			receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZBONO_TXMINT_GENERAL);
			return false;
		}
		else if (mapBlockIndex.count(hashBlock) < 1) {
			// check that this mint made it into the blockchain
			receipt.SetStatus(_("Mint did not make it into blockchain"), ZBONO_TXMINT_GENERAL);
			return false;
		}
		...
		if (outputIndex == -1) {
			receipt.SetStatus(_("Pubcoin not found in mint tx"), ZBONO_TXMINT_GENERAL);
			return false;
		}

		mint.SetOutputIndex(outputIndex);
		CTxIn in;
		if (!ZBONOModule::createInput(in, mint, hashTxOut, spendVersion)) {
			receipt.SetStatus(_("Cannot create public spend input"), ZBONO_TXMINT_GENERAL);
			return false;
		}
		vin.emplace_back(in);
...
std::string CWallet::GetUniqueWalletBackupName(bool fzbonoAuto) const
{
