...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BONO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBONO send
enum ZerocoinSpendStatus {
    ZBONO_SPEND_OKAY = 0,                            // No error
    ZBONO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBONO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBONO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBONO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBONO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBONO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBONO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBONO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBONO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBONO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBONO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBONO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBONO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBONO_SPENT_USED_ZBONO = 14,                      // Coin has already been spend
    ZBONO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBONO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    static const int STAKE_SPLIT_THRESHOLD = 50;
    ...
    bool CreateZBONOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);

    std::string GetUniqueWalletBackupName(bool fzbonoAuto) const;
    ...
    */
    ...
    /// Get 10000 BONO output and keys which can be used for the Masternode
