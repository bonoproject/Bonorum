...
public:
    ...
    bool WriteZBONOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBONOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBONOSeed_deprecated(uint256& seed);
    bool EraseZBONOSeed();
    bool EraseZBONOSeed_deprecated();

    bool WriteZBONOCount(const uint32_t& nCount);
    bool ReadZBONOCount(uint32_t& nCount);
