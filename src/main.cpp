...
#include "zbono/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "BonorumCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubono) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
		return error("%s : zBONO spend with serial %s is already in block %d\n", __func__,
...
{
	//Check to see if the zBONO is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		try {
			if (!spend->HasValidSignature())
				return error("%s: V2 zBONO spend does not have a valid signature\n", __func__);
		}
		...
		if (spend->getSpendType() != expectedType) {
			return error("%s: trying to spend zBONO without the correct spend type. txid=%s\n", __func__,
				tx.GetHash().GetHex());
		}
	}
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state)
{
	...
	for (const CTxIn& txin : tx.vin) {
		...
		if (isPublicSpend) {
			...
			if (!ZBONOModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
		...
		if (isPublicSpend) {
			...
			if (!ZBONOModule::validateInput(txin, prevOut, tx, ret)) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
	...
	if (!CheckTransaction(tx, chainHeight >= Params().Zerocoin_StartHeight(), state, fColdStakingActive))
...

	{
		...
		if (hasZcSpendInputs) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("%s : zBONO spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!isPrivZerocoinSpend && !isPublicSpend) {
					return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
						__func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbono");
				}
				...
				if (isPublicSpend) {
					libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
					PublicCoinSpend publicSpend(params);
					if (!ZBONOModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
						return false;
					}
					...
					if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
						return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
							__func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zbono");
					}
			...
			// Check that zBONO mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 37703116 && nHeight > 300000) {
		nSubsidy = 0.92 * COIN;
	}
	else if (nHeight == 37703117) {
		nSubsidy = 0.18 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBONOStake)
{
	int64_t ret = 0;
	...
	if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 37703117) {
		ret = blockValue * 0.9;
	}
	else {
		ret = 0;
		...
		for (const CTxIn& in : tx.vin) {
			...
			if (in.IsZerocoinSpend() || isPublicSpend) {
				...
				if (isPublicSpend) {
					...
					if (!ZBONOModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		 ...
		 * note we only undo zerocoin databasing in the following statement, value to and from BonorumCoin
...
{
	RenameThread("bonorumcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		if (tx.HasZerocoinSpendInputs()) {
			...
			// Check that zBONO mints are not already known
			if (tx.HasZerocoinMintOutputs()) {
				for (auto& out : tx.vout) {
					if (!out.IsZerocoinMint())
						continue;
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
	...
	if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
		...
		// cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 BONO.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		if (!CheckTransaction(tx, fZerocoinActive, state, fColdStakingActive))
			return error("%s : CheckTransaction failed", __func__);

		// double check that there are no double spent zBONO spends in this block
		if (tx.HasZerocoinSpendInputs()) {
			for (const CTxIn& txIn : tx.vin) {
				bool isPublicSpend = txIn.IsZerocoinPublicSpend();
				if (txIn.IsZerocoinSpend() || isPublicSpend) {
					...
					if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zBONO serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	if (block.nBits != nBitsRequired) {
		// BonorumCoin Specific reference to the block with the wrong threshold was used.
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> bonoInputs;
		std::vector<CTxIn> zBONOInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.IsZerocoinSpend()) {
				zBONOInputs.push_back(stakeIn);
			}
			else {
				bonoInputs.push_back(stakeIn);
			}
		}
		const bool hasBONOInputs = !bonoInputs.empty();
		const bool hasZBONOInputs = !zBONOInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasBONOInputs) {
					...
					for (const CTxIn& bonoIn : bonoInputs) {
						if (bonoIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zBONO inputs.
			if (hasZBONOInputs) {
				for (const CTxIn& zBonoInput : zBONOInputs) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zBonoInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbono");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// BonorumCoin: We use certain sporks during IBD, so check to see if they are
		...
		if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
			try {
				...
				if (!GetAccumulatorValue(height, den, bnAccValue)) {
					LogPrint("zbono", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
					Misbehaving(pfrom->GetId(), 50);
				}
				else {
			...
			try {
				...
				if (gen.isValid(chainActive.Height())) {
					if (!lightWorker.addWitWork(gen)) {
						LogPrint("zbono", "%s : add genwit request failed \n", __func__);
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
